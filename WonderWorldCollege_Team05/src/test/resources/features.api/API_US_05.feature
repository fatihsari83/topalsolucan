...
Feature: US05 As an administrator, I should be able to delete the visitor puspose record in the system via the API connection.
  ...
  Scenario: TC01 When a DELETE body containing valid authorization information and correct data (id) is sent to the api/visitorsPurposeDelete endpoint, it should be verified that the status code returned is 200 and the message information in the response body is "Success"
  ...
  Scenario: TC02 When a DELETE body containing invalid authorization information or wrong data (id) is sent to the api/visitorsPurposeDelete endpoint, it should be verified that the status code returned is 403 and the message information in the response body is "failed"
