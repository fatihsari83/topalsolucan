Feature: US_34 As an administrator, I want to access the Visitor information of a visitor with a given ID through API connection.
  ...
  Scenario: TC_01 When a POST body with valid authorization information and correct data (id) is sent to the api/visitorsId endpoint,
            it should be verified that the status code returned is 200 and in the response body is "Success".
  ...
  Scenario: TC_02 When a POST body containing invalid authorization information or invalid data (id) is sent to the api/visitorsId endpoint,
            it should be verified that the status code returned is 403 and response body is "failed".
