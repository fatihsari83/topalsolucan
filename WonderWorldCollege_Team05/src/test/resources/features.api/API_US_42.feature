Feature: US42 As an administrator, I want to be able to delete a Notice record from the system through API connection.
  ...
  Scenario: TC01 When a valid authorization and correct data (id) are sent with a DELETE body to the api/deleteNotice endpoint,
    ...
    * For admin, verifies that the message information is "Success"
    ...
    Scenario: TC02 When invalid authorization or incorrect data (id) is sent with a DELETE body to the api/deleteNotice endpoint, the response
