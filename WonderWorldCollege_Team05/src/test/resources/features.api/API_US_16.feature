...
Feature: US16 As an administrator, I want to update the registered Vehicle information in the system through API connection
  ...
  Scenario: TC02 Status code returned when sending a PATCH body (vehicle_no, vehicle_model, vehicle_photo, manufacture_year, registration_number, chasis_number, max_seating_capacity, driver_name, driver_licence, driver_contact, note) containing invalid authorization information or missing/wrong data (id) to the api/vehicleUpdate endpoint It should be verified that it is 403 and the message information in the response body is "failed"
