Feature: US_38 As an administrator, I want to access the Notice List through API connection.
  ...
  * Records response for Admin with valid authorization information
  ...
  Examples:
    | expectedData     |
    |id=34type=noticeslug=wonder-world-college-receives-accreditation-for-its-outstanding-business-programtitle=Wonder World College Receives Accreditation for its Outstanding Business Programdate=2023-07-04event_start=nullevent_end=nullevent_venue=nulldescription=notice denememeta_title=meta_description=meta_keyword=feature_image=publish_date=publish=sidebar= |
