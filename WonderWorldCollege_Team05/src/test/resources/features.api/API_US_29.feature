Feature: US_29 As an administrator, I want to access the book information of a book with a given ID through API connection.
  ...
  Scenario: TC_01 When a valid authorization information and correct data (id)
  are sent with a POST body to the api/booksId endpoint, the expected status code is 200, and the response body's message should be "Success."
  ...
  Scenario: TC_02 When invalid authorization information or invalid data (id)
  is sent with a POST body to the api/booksId endpoint, the expected status code is 403, and the response body's message should be "failed."
